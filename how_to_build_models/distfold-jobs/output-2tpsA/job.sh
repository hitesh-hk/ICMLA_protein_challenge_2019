...
export CNS_CUSTOMMODULE=/home/SSD/protien_ICMLA/code_4/IEEE-ICMLA-2019-PIDP-Challenge/how_to_build_models/distfold-jobs/output-2tpsA/                 
...
if [ -f "2tpsA_20.pdb" ]; then           
   rm iam.running                                 
   echo "trial structures written."             
   rm *embed*                                     
   exit                                           
fi                                                
if [ -f "2tpsAa_20.pdb" ]; then 
   rm iam.running                                 
