...
export CNS_CUSTOMMODULE=/home/SSD/protien_ICMLA/code_4/IEEE-ICMLA-2019-PIDP-Challenge/how_to_build_models/distfold-jobs/output-1p90A/                 
...
if [ -f "1p90A_20.pdb" ]; then           
   rm iam.running                                 
   echo "trial structures written."             
   rm *embed*                                     
   exit                                           
fi                                                
if [ -f "1p90Aa_20.pdb" ]; then 
   rm iam.running                                 
